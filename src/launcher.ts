 ...
 *   2. If --setup flag or first run (no .env): run the setup wizard, then exit
 *   3. Check stored version against package version; if different, run
 *      profile-aware migration (post-update.mjs) and update the version sentinel
 *   4. Start the MCP server inline
 *
...
// ── Setup wizard ──────────────────────────────────────────────────────────────
if (wantSetup || isFirstRun) {
    if (isFirstRun) {
        mkdirSync(MODLENS_HOME, { recursive: true });
        console.error("[modlens] First run — launching setup wizard...\n");
    }
