...
OAuth clients, authorization state, grants, and token hashes are stored in the configured database. `MODLENS_OAUTH_CODEX_REFRESH_WORKAROUND` defaults to `1`, issuing 30-day ModLens access tokens to avoid frequent client-side refresh-token rotation while [Codex's stale-credential issue](https://github.com/openai/codex/issues/17265) remains. Set it to `0` to restore short-lived access tokens without changing the upstream refresh implementation; this immediately rejects previously issued long-lived tokens, so connected clients must refresh or sign in again. The temporary long-lived branch in `issue()` can then be removed. A leaked 30-day ModLens bearer token remains usable for longer unless revoked. ModLens refreshes its upstream provider token server-side as needed and still checks the account profile on every authenticated request. A multi-replica deployment must point every replica at the same persistent database and use the same `MODLENS_OAUTH_STORAGE_KEY`; the embedded SQLite default is suitable only for a single replica. Upstream token refreshes are coalesced within a process, so use one replica when the upstream provider rotates refresh tokens.

Set `MODLENS_OAUTH_LOG=1` to write structured OAuth lifecycle events to stderr; they are disabled by default. Failures include an `incident` reference shown on browser-facing error pages, which can be matched to a server log when diagnostics are enabled. These events include the stage, route, status, error code, hashed client reference, and redirect origin where relevant; authorization codes, state values, PKCE material, tokens, provider profiles, and subjects are never logged. Unexpected server errors remain logged independently of this switch.
